...
const PAGE_SLUG = "emergency-support";
...
export async function GET() {
  try {
    ...
    console.error("Error fetching emergency-support content:", error);
...
export async function PUT(request: Request) {
  try {
    ...
    const page = await prisma.page.upsert({
      where: { slug: PAGE_SLUG },
      create: {
        title: "24/7 Emergency Power Support",
        slug: PAGE_SLUG,
        type: "static",
    ...
    console.error("Error saving emergency-support section:", error);
    return NextResponse.json(
      { success: false, error: "Internal Server Error" },
      { status: 500 }
    );
