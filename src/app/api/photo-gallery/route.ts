...
export async function GET() {
  try {
    ...
    console.error("Error fetching photo gallery page content:", error);
...
export async function PUT(request: Request) {
  try {
    ...
    const sectionType = section || body.type || "gallery";
    ...
    console.error("Error saving photo gallery page section:", error);
    return NextResponse.json(
      { success: false, error: "Internal Server Error" },
      { status: 500 }
    );
