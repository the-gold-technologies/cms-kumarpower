...
const SECTION_TYPE = "kirloskar-gas-generator";

const defaultGensets: GasGensetCard[] = [
  {
    id: "gg-1",
...
export default function KirloskarGasGeneratorCMSPage() {
  ...
  // Hero Section
  const [heroHeadingPart1, setHeroHeadingPart1] = useState("Kirloskar Gas Generators");
  ...
  // Gensets List
  const [gensets, setGensets] = useState<GasGensetCard[]>(defaultGensets);
  ...
  const saveAllToDB = async () => {
    ...
    if (res.ok) {
      ...
      toast.success("Kirloskar Gas Generator page updated!");
    } else {
      toast.error("Failed to save Kirloskar Gas Generator page");
    }
  };
  ...
  const addGenset = () => {
    ...
    toast.success("New Gas Generator range added!");
  };

  const removeGenset = (id: string) => {
    setGensets((prev) => prev.filter((g) => g.id !== id));
    toast.success("Gas Generator range removed");
  };

  return (
    <div className="flex flex-col gap-6 pb-12">
      <PageHeader
        title="Kirloskar Gas Generators CMS (/products/kirloskar-gas-generator)"
        description="Manage banner text, natural gas & CNG generator models, technical specs & brochures."
      />
        ...
        <div className={`grid transition-all duration-300 ${isHeroOpen ? "grid-rows-[1fr] opacity-100 mt-6" : "grid-rows-[0fr] opacity-0 mt-0 pointer-events-none"}`}>
          <div className="overflow-hidden flex flex-col gap-4 pt-1">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputField label="Hero Heading (Regular Part)" value={heroHeadingPart1} onChange={(e) => setHeroHeadingPart1(e.target.value)} placeholder="Kirloskar Gas Generators" />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
              <InputField label="Section Title" value={sectionTitle} onChange={(e) => setSectionTitle(e.target.value)} placeholder="Gas Generators" />
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm ring-1 ring-gray-100/50">
        <SectionHeader
          title={`2. Gas Generator Power Ranges (${gensets.length} Ranges)`}
          description="Manage gas generator models, technical specs & brochure downloads."
              ...
              {gensets.map((g, idx) => (
                <div key={g.id} className="p-5 bg-slate-50 border border-slate-200 rounded-2xl space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <InputField label="Name" value={g.name} onChange={(e) => handleGensetChange(g.id, "name", e.target.value)} placeholder="e.g. 15 kVA to 250 kVA Gas generators" />
                    <InputField label="Power Range String" value={g.range} onChange={(e) => handleGensetChange(g.id, "range", e.target.value)} placeholder="15 kVA to 250 kVA" />
