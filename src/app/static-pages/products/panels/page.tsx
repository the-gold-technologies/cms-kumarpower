...
export default function ElectricalPanelsCMSPage() {
  ...
  const saveAllToDB = async () => {
    const rawPayload = {
      ...
      sectionDesc:
        "Browse our range of AMF panels, synchronization panels, and custom power distribution boards.",
      panels,
      whyChooseTitle,
      whyChooseCard1Title,
      whyChooseCard1Desc,
  ...
  return (
    <div className="flex flex-col gap-6 pb-12">
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm ring-1 ring-gray-100/50">
        ...
        />
        <div
          className={`grid transition-all duration-300 ${isPanelsOpen ? "grid-rows-[1fr] opacity-100 mt-6" : "grid-rows-[0fr] opacity-0 mt-0 pointer-events-none"}`}
        >
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <InputField
                      ...
                      value={p.name}
                      onChange={(e) =>
                        handlePanelChange(p.id, "name", e.target.value)
                      }
                      placeholder="e.g. AMF Panels"
                    />
                    <InputField
                      label="Rating / Capacity Range"
                      value={p.range}
                      onChange={(e) =>
                        handlePanelChange(p.id, "range", e.target.value)
                      }
                      placeholder="Various"
                    />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <ImageUploadField
                      label="Panel Graphic Image"
                      value={p.image}
                      onChange={(val) => handlePanelChange(p.id, "image", val)}
                    />
                    <PDFUploadField
                      label="Brochure PDF Document"
                      value={p.brochurePdf}
                      onChange={(val) =>
                        handlePanelChange(p.id, "brochurePdf", val)
                      }
                    />
                  </div>
                  <TextAreaField
                    ...
                    value={p.description}
                    onChange={(e) =>
                      handlePanelChange(p.id, "description", e.target.value)
                    }
                    rows={2}
                  />
                  <TextAreaField
                    ...
                    onChange={(e) =>
                      handlePanelChange(p.id, "technicalSpecs", e.target.value)
                    }
                    rows={4}
                  />
                </div>
