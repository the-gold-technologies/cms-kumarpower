...
export default function DistributionTransformersCMSPage() {
  ...
  const saveAllToDB = async () => {
    ...
    if (res.ok) {
      ...
      toast.success("Transformers page updated!");
    } else {
      toast.error("Failed to save Transformers page");
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 pb-12">
      <PageHeader
        ...
        description="Manage banner text, transformer models, why choose benefits, certification badges, and consultation helpline text."
      />
      ...
      <div className="bg-white rounded-2xl p-8 shadow-sm ring-1 ring-gray-100/50">
        ...
        />
        <div
          className={`grid transition-all duration-300 ${isTransOpen ? "grid-rows-[1fr] opacity-100 mt-6" : "grid-rows-[0fr] opacity-0 mt-0 pointer-events-none"}`}
        >
              ...
              {transformers.map((t, idx) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      onClick={() => removeTransformer(t.id)}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <InputField
                      ...
                      value={t.name}
                      onChange={(e) =>
                        handleTransChange(t.id, "name", e.target.value)
                      }
                      placeholder="e.g. Distribution Transformers"
                    />
                    <InputField
                      label="Capacity Range"
                      value={t.range}
                      onChange={(e) =>
                        handleTransChange(t.id, "range", e.target.value)
                      }
                      placeholder="100-2500 kVA"
                    />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <ImageUploadField
                      label="Transformer Graphic Image"
                      value={t.image}
                      onChange={(val) => handleTransChange(t.id, "image", val)}
                    />
                    <PDFUploadField
                      label="Brochure PDF Document"
                      value={t.brochurePdf}
                      onChange={(val) =>
                        handleTransChange(t.id, "brochurePdf", val)
                      }
                    />
                  </div>
                  <TextAreaField
                    ...
                    value={t.description}
                    onChange={(e) =>
                      handleTransChange(t.id, "description", e.target.value)
                    }
                    rows={2}
                  />
                  <TextAreaField
                    label="Technical Specifications Detail"
                    value={t.technicalSpecs}
                    onChange={(e) =>
                      handleTransChange(t.id, "technicalSpecs", e.target.value)
                    }
                    rows={4}
                  />
                </div>
        ...
        >
          <div className="overflow-hidden flex flex-col gap-6 pt-1">
            <InputField
              ...
              placeholder="Why Choose Kumar Power Transformers?"
            />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
