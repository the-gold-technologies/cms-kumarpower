...
export const InputField: React.FC<InputFieldProps> = ({
  ...
  const inputClass = `w-full ${
    icon ? "pl-12" : "px-5"
  } py-3.5 bg-white border border-gray-200 rounded-2xl text-sm focus:ring-2 focus:ring-[#2D6FBA]/30 focus:border-[#2D6FBA] outline-none transition-all text-gray-800 placeholder:text-gray-300 ${className}`;
  ...
  return (
    <div className={`flex flex-col gap-2.5 ${containerClassName} px-0.5`}>
      {label && (
        <label className="text-[11px] font-extrabold text-gray-400 uppercase tracking-widest ml-1 flex items-center gap-1.5 relative">
          {label}
          {tooltip && (
